...
def test(args):
    ...
    if model_name in ["pspnet", "icnet", "icnetBN"]:
        ...
        img = resize(img, (orig_size[0] // 2 * 2 + 1, orig_size[1] // 2 * 2 + 1), preserve_range=True)
    else:
        img = resize(img, (loader.img_size[0], loader.img_size[1]), preserve_range=True)

    img = img[:, :, ::-1]
    img = img.astype(np.float64)
    img -= loader.mean_rgb['pascal']
    ...
    if model_name in ["pspnet", "icnet", "icnetBN"]:
        ...
        pred = np.round(resize(pred, orig_size))

    decoded = loader.decode_segmap(pred)
    print("Classes found: ", np.unique(pred))
    io.imwrite(args.out_path, decoded)
